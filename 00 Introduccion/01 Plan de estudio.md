Índice de Temas: Algoritmos y Estructuras de Datos

1. Fundamentos

    Introducción a algoritmos y complejidad
    Notación asintótica (Big O, Omega, Theta)
    Análisis de complejidad temporal y espacial
    Recursividad y relaciones de recurrencia
    Técnicas de análisis (mejor caso, peor caso, caso promedio)

2. Estructuras de Datos Lineales

    Arreglos (arrays) estáticos y dinámicos
    Listas enlazadas (simples, dobles, circulares)
    Pilas (stacks)
    Colas (queues, deques, colas de prioridad)

3. Estructuras de Datos No Lineales
Árboles

    Árboles binarios
    Árboles binarios de búsqueda (BST)
    Árboles balanceados (AVL, Rojo-Negro)
    Árboles B y B+
    Tries (árboles de prefijos)
    Segment trees y Fenwick trees (BIT)
    Heaps (montículos) binarios y binomiales

Grafos

    Representación de grafos (listas y matrices de adyacencia)
    Recorridos: BFS y DFS
    Grafos dirigidos y no dirigidos
    Detección de ciclos
    Ordenamiento topológico

4. Estructuras Hash

    Tablas hash
    Funciones hash
    Manejo de colisiones (encadenamiento, direccionamiento abierto)
    Hash sets y hash maps

5. Algoritmos de Ordenamiento

    Ordenamientos simples: burbuja, inserción, selección
    Ordenamientos eficientes: merge sort, quicksort, heapsort
    Ordenamientos no comparativos: counting sort, radix sort, bucket sort
    Comparación de estabilidad y complejidad

6. Algoritmos de Búsqueda

    Búsqueda lineal
    Búsqueda binaria
    Búsqueda en estructuras específicas (BST, hash)

7. Técnicas de Diseño de Algoritmos

    Divide y vencerás
    Programación dinámica (memoización, tabulación)
    Algoritmos codiciosos (greedy)
    Backtracking
    Branch and bound
    Two pointers y sliding window

8. Algoritmos sobre Grafos Avanzados

    Caminos más cortos: Dijkstra, Bellman-Ford, Floyd-Warshall
    Árboles de expansión mínima: Kruskal, Prim
    Flujo máximo (Ford-Fulkerson, Edmonds-Karp)
    Componentes fuertemente conexas (Tarjan, Kosaraju)
    Union-Find (Disjoint Set Union)

9. Estructuras de Datos Avanzadas

    Skip lists
    Sparse tables
    Estructuras persistentes
    Estructuras probabilísticas (Bloom filters, Skip Lists)

10. Cadenas (Strings)

    Búsqueda de patrones (KMP, Rabin-Karp, Boyer-Moore)
    Árboles de sufijos y arreglos de sufijos
    Distancia de edición (Levenshtein)

11. Algoritmos Numéricos y Matemáticos

    Algoritmo de Euclides (MCD)
    Exponenciación rápida
    Números primos (Criba de Eratóstenes)
    Combinatoria y teoría de números básica

12. Complejidad Computacional

    Clases P y NP
    Problemas NP-completos
    Heurísticas y aproximaciones